...
class Wireless:
    ...
    def setAPaddr(self, addr):
        ...
        if addr == "AUTO" or addr == "ANY":
            mac_addr = "\xFF" * wififlags.ETH_ALEN
        elif addr == "OFF":
            mac_addr = "\x00" * wififlags.ETH_ALEN
        else:
            if ":" not in addr:
        ...
        status, result = self.iwstruct.iw_set_ext(
            self.ifname, wififlags.SIOCSIWAP, iwreq
        )
    ...
    def setEssid(self, essid):
        ...
        """
        if len(essid) > wififlags.IW_ESSID_MAX_SIZE:
            raise OverflowError(errno.EOVERFLOW, os.strerror(errno.EOVERFLOW))
        iwpoint = Iwpoint(essid, 1)
        status, result = self.iwstruct.iw_set_ext(
            self.ifname, wififlags.SIOCSIWESSID, data=iwpoint.packed_data
        )
    ...
    def getEncryption(self):
        ...
        iwpoint = Iwpoint("\x00" * wififlags.IW_ENCODING_TOKEN_MAX)
        status, result = self.iwstruct.iw_get_ext(
            self.ifname, wififlags.SIOCGIWENCODE, data=iwpoint.packed_data
        )
        iwpoint.update(result)

        if iwpoint.flags & wififlags.IW_ENCODE_NOKEY > 0:
            return "**" * iwpoint.length
        elif iwpoint.flags & wififlags.IW_ENCODE_OPEN > 0:
            return "open"
        elif iwpoint.flags & wififlags.IW_ENCODE_RESTRICTED > 0:
            return "restricted"
        elif iwpoint.flags & wififlags.IW_ENCODE_DISABLED > 0:
            return "off"
    ...
    def setEncryption(self, mode):
        ...
        numeric_mode = (
            numeric_mode
            & ~wififlags.IW_ENCODE_OPEN
            & ~wififlags.IW_ENCODE_RESTRICTED
            & ~wififlags.IW_ENCODE_DISABLED
        )
        if (mode == "OPEN") or (mode == wififlags.IW_ENCODE_OPEN):
            numeric_mode = numeric_mode | wififlags.IW_ENCODE_OPEN
        elif (mode == "RESTRICTED") or (mode == wififlags.IW_ENCODE_RESTRICTED):
            numeric_mode = numeric_mode | wififlags.IW_ENCODE_RESTRICTED
        elif (mode == "OFF") or (mode == wififlags.IW_ENCODE_DISABLED):
            numeric_mode = numeric_mode | wififlags.IW_ENCODE_DISABLED
        iwpoint = Iwpoint("\x00" * wififlags.IW_ENCODING_TOKEN_MAX, numeric_mode)
        status, result = self.iwstruct.iw_get_ext(
            self.ifname, wififlags.SIOCSIWENCODE, data=iwpoint.packed_data
        )
    ...
    def setKey(self, key, index=1):
        ...
        """
        if index not in range(1, wififlags.IW_ENCODE_INDEX):
            raise IndexError
        ...
        iwpoint = Iwpoint(cooked_key, index + wififlags.IW_ENCODE_ENABLED)
        status, result = self.iwstruct.iw_get_ext(
            self.ifname, wififlags.SIOCSIWENCODE, data=iwpoint.packed_data
        )
    ...
    def setFrequency(self, freq):
        ...
        iwstruct = Iwstruct()
        if freq == "auto":
            iwreq = iwstruct.pack(format, -1, 0, 0, wififlags.IW_FREQ_AUTO)
        else:
            if freq == "fixed":
            ...
            if freq_match is None:
                ...
                freq_pattern = re.compile(r"([\d\.]+)", re.I | re.M | re.S)
                freq_match = freq_pattern.search(freq)
            ...
            if e > 8:
                ...
                m = int(freq_num)
                e = 0
            iwreq = iwstruct.pack(format, m, e, 0, wififlags.IW_FREQ_FIXED)
        status, result = iwstruct.iw_set_ext(self.ifname, wififlags.SIOCSIWFREQ, iwreq)
    ...
    def getMode(self):
        ...
        """
        return wififlags.modes[self.wireless_info.getMode()]

    def setMode(self, mode):
        """Sets the operation mode."""
        this_modes = [x.lower() for x in wififlags.modes]
        mode = mode.lower()
        try:
        ...
        status, result = self.iwstruct.iw_set_ext(
            self.ifname, wififlags.SIOCSIWMODE, data=datastr
        )
    ...
    def getStatistics(self):
        ...
        return [iwstats.status, iwstats.qual, iwstats.discard, iwstats.missed_beacon]
    ...
    def commit(self):
        ...
        status, result = self.iwstruct.iw_set_ext(self.ifname, wififlags.SIOCSIWCOMMIT)
...
class WirelessConfig:
    ...
    def getEncryption(self):
        ...
        # use an IW_ENCODING_TOKEN_MAX-cell array of NULLs
        #   as space for ioctl to write encryption info
        iwpoint = Iwpoint(b"\x00" * wififlags.IW_ENCODING_TOKEN_MAX)
        status, result = self.iwstruct.iw_get_ext(
            self.ifname, wififlags.SIOCGIWENCODE, data=iwpoint.packed_data
        )
        iwpoint.update(result)
    ...
    def getFrequency(self):
        ...
        """
        status, result = self.iwstruct.iw_get_ext(self.ifname, wififlags.SIOCGIWFREQ)
    ...
    def getKey(self, key=0):
        ...
        # use an IW_ENCODING_TOKEN_MAX-cell array of NULLs
        #   as space for ioctl to write encryption info
        iwpoint = Iwpoint("\x00" * wififlags.IW_ENCODING_TOKEN_MAX, key)
        status, result = self.iwstruct.iw_get_ext(
            self.ifname, wififlags.SIOCGIWENCODE, data=iwpoint.packed_data
        )
        iwpoint.update(result)
    ...
    def getEssid(self):
        ...
        iwpoint = Iwpoint(b"\x00" * wififlags.IW_ESSID_MAX_SIZE)
        status, result = self.iwstruct.iw_get_ext(
    ...
    def getMode(self):
        ...
        """
        status, result = self.iwstruct.iw_get_ext(self.ifname, wififlags.SIOCGIWMODE)
...
class WirelessInfo(WirelessConfig):
    ...
    def getSensitivity(self):
        ...
        """
        return Iwparam(self.ifname, wififlags.SIOCGIWSENS)
    ...
    def getAPaddr(self):
        ...
        status, result = self.iwstruct.iw_get_ext(
            self.ifname, wififlags.SIOCGIWAP, data=datastr
        )
    ...
    def getBitrate(self):
        ...
        """
        return Iwparam(self.ifname, wififlags.SIOCGIWRATE)
    ...
    def getRTS(self):
        ...
        """
        return Iwparam(self.ifname, wififlags.SIOCGIWRTS)
    ...
    def getFragmentation(self):
        ...
        """
        return Iwparam(self.ifname, wififlags.SIOCGIWFRAG)
    ...
    def getPower(self):
        ...
        """
        return Iwparam(self.ifname, wififlags.SIOCGIWPOWER)
    ...
    def getTXPower(self):
        ...
        """
        return Iwparam(self.ifname, wififlags.SIOCGIWTXPOW)
...
class Iwstruct:
    ...
    def iw_get_ext(self, ifname, request, data=None):
        ...
        return (result, ifreq[wififlags.IFNAMSIZE :])
...
class Iwparam:
    ...
    def _parse(self, data):
        ...
        iwstruct = Iwstruct()
        self.value, self.fixed, self.disabled, self.flags = iwstruct.parse_data(
            self.fmt, data
        )
...
class Iwstats:
    ...
    def update(self):
        ...
        i, result = iwstruct.iw_get_ext(self.ifname, wififlags.SIOCGIWSTATS, data=s)
        if i > 0:
            self.error = result
            self.errorflag = i
    ...
    def _parse(self, data):
        ...
        self.discard = makedict(
            nwid=nwid, code=code, fragment=frag, retries=retries, misc=iwflags
        )
...
class Iwpoint:
    ...
    def update(self, packed_data):
        ...
        self.caddr_t, self.length, self.flags = struct.unpack(
            self.fmt, self.packed_data
        )
...
class Iwscan:
    ...
    def __init__(self, ifname, fullscan=True):
        ...
        if fullscan:
            ...
            status, result = iwstruct.iw_set_ext(
                self.ifname, wififlags.SIOCSIWSCAN, datastr
            )
            self.getScan()
    ...
    def getScan(self):
        ...
        iwstruct = Iwstruct()
        bufflen = wififlags.IW_SCAN_MAX_DATA
            ...
            try:
                ...
                if io_error.errno == errno.E2BIG:
                    ...
                    if bufflen < newlen:
                        ...
                        bufflen = bufflen * 2
                elif io_error.errno == errno.EAGAIN:
...
class Iwscanresult:
    ...
    def addEvent(self, cmd, data):
        ...
        """
        if (cmd in range(wififlags.SIOCIWFIRST, wififlags.SIOCIWLAST + 1)) or (
            cmd in range(wififlags.IWEVFIRST, wififlags.IWEVLAST + 1)
        ):
            if cmd == wififlags.SIOCGIWNWID:
                pass
            elif cmd == wififlags.SIOCGIWFREQ:
                self.frequency = Iwfreq(data)
            elif cmd == wififlags.SIOCGIWMODE:
                raw_mode = struct.unpack("I", data[:4])[0]
                self.mode = wififlags.modes[raw_mode]
            elif cmd == wififlags.SIOCGIWNAME:
                self.protocol = data[: len(data) - 2]
            elif cmd == wififlags.SIOCGIWESSID:
                self.essid = data[4:]
                ...
                self.encode.update(
                    struct.pack(
                        "PHH",
                        (int(data[0]) << 16) + int(data[1]),
                        data[2] << 8,
                        data[3] << 8,
                    )
                )
                if self.encode.caddr_t is None:
                    self.encode.flags = self.encode.flags | wififlags.IW_ENCODE_NOKEY
            elif cmd == wififlags.SIOCGIWRATE:
            ...
            elif cmd == wififlags.SIOCGIWMODUL:
                pass
                ...
                raise ValueError(
                    "Unknown IW event command received. This "
                    + "command cannot be used to add information "
                    + "to the WiFi cell's profile."
                )
        else:
            raise ValueError(
                "Invalid IW event command received.  \
